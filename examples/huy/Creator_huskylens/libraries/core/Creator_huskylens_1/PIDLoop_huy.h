...
int m_speed;
  ...
  // setSpeed(int set_speed)
  // {
  //   m_speed = set_speed;
  //   return m_speed;
  // }
  void set_Kp(int32_t pgain)
  {
    m_pgain = pgain;
  }
  void set_Ki(int32_t igain)
  {
    m_igain = igain;
  }
  void set_Kd(int32_t dgain)
  {
    m_dgain = dgain;
  }
...
void Serial_update_pid(PIDLoop *pPid)
{
  if (Serial.available() > 0)
  {
    ...
    {
      int tempKd = pPid->Get_Kd();
      int tempKi = pPid->Get_Ki();
      int tempKp = tempNum;
      pPid->set_Kp(tempKp);

      DEBUG_SERIAL_CMD_T(F("Set kp"));
      DEBUG_SERIAL_CMD_LN(tempKp);
      DEBUG_SERIAL_CMD_T(F("Set ki"));
      DEBUG_SERIAL_CMD_LN(tempKi);
    ...
    else if (tempString.indexOf("kt") > -1)
    {
    ...
    //   int Set_speed = tempNum;
    //   pPid->setSpeed(Set_speed);

    //   DEBUG_SERIAL_CMD_T(F("Set motorOffset: "));
    //   DEBUG_SERIAL_CMD_LN(Set_speed);
    // }
  }
}